...
class ItemSimilarityMatrixBuilder(object):
    ...
    def build(self, ratings, save=True):
        ...
        ratings['food_id'] = ratings['food_id'].astype('category')

        coo = coo_matrix((ratings['avg'].astype(float),
                          (ratings['food_id'].cat.codes.copy(),
        ...
        if save:
            ...
            if self.db == 'django.db.backends.postgresql':
                self._save_similarities(cor, foods)
            else:
                logger.debug('save starting with django')
                self._save_with_django(cor, foods)
    ...
    def _save_with_django(self, sm, index, created=datetime.now()):
        ...
        for x, y in zip(xs, ys):
            ...
            new_similarity = Similarity(
                ...
                target=index[y],
                created=created,
                similarity=decimal.Decimal(sim),
            )
            no_saved += 1
            sims.append(new_similarity)
...
def load_all_ratings(min_ratings=1):
    columns = ['user_id', 'food_id', 'rating', 'type']

    ratings_data = Rating.objects.all().values(*columns)
    ratings = pd.DataFrame.from_records(ratings_data, columns=columns)
    user_count = ratings[['user_id', 'food_id']].groupby('user_id').count()
    user_count = user_count.reset_index()
    user_ids = user_count[user_count['food_id'] > min_ratings]['user_id']
